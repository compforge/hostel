...
func (p *testParticipant) Release(context.Context) error {
...
func TestSequenceRetriesOnlyUnfinishedHooks(t *testing.T) {
	...
	if err := sequence.Run(context.Background()); err == nil {
...
func TestSequenceCancellationDoesNotAdvance(t *testing.T) {
	...
	cancel()
	if err := sequence.Run(ctx); !errors.Is(err, context.Canceled) {
		t.Fatal(err)
	}
	if len(log) != 0 {
		t.Fatal(log)
