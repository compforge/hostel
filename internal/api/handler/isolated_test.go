...
func createIsolatedSession(t *testing.T, s *testServer) string {
	t.Helper()
	...
	var response apiview.IsolatedCreate
...
func runIsolated(t *testing.T, s *testServer, sessionID, body string) []apiview.StreamEvent {
	t.Helper()
...
func TestIsolatedSessionLifecycleAndStatefulRun(t *testing.T) {
	...
	var state apiview.IsolatedSessionState
	...
	if events[len(events)-1].Type != apiview.EventExecutionEnd {
		t.Fatalf("first run events = %+v", events)
	}
	events = runIsolated(t, s, id, `{"code":"echo \"$SESSION_TEST_STATE\""}`)
	if !isolatedOutputContains(events, "kept") || events[len(events)-1].Type != apiview.EventExecutionEnd {
...
func isolatedOutputContains(events []apiview.StreamEvent, want string) bool {
	for _, event := range events {
		if event.Type == apiview.EventStdout && strings.Contains(event.Text, want) {
			return true
		}
	}
	return false
...
func TestIsolatedUnsupportedOptionsAndCapabilities(t *testing.T) {
	...
	var response apiview.ErrorResponse
	if err := json.Unmarshal(rec.Body.Bytes(), &response); err != nil || response.Code != apiview.ErrNotSupported {
...
func TestIsolatedWorkdirContract(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var created apiview.IsolatedCreate
