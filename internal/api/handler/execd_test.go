...
func decodeExecd(t *testing.T, body string) []execdStreamEvent {
	t.Helper()
	var events []execdStreamEvent
	for _, line := range strings.Split(body, "\n") {
		if !strings.HasPrefix(line, "data: ") {
			continue
		}
		var ev execdStreamEvent
...
func TestExecdFilesUseResolvedBed(t *testing.T) {
	...
	for _, base := range []string{"/execd", "/v1/beds/a/execd"} {
		...
		_, _ = md.Write([]byte(`{"path":"/tmp/中文 '/file.bin","mode":384}`))
