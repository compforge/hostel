...
func (s *Handler) fileReader(ops *bedfs.FS) *bedfs.Reader {
	fallbackRoot := ""
...
func (s *Handler) filesInfo(c *gin.Context) {
...
func (s *Handler) filesChmod(c *gin.Context) {
	_, ops, finishOperation := s.opsOf(c)
	if ops == nil {
		return
	}
...
func (s *Handler) filesSearch(c *gin.Context) {
	_, ops, finishOperation := s.opsOf(c)
	if ops == nil {
		return
	}
...
func (s *Handler) filesUpload(c *gin.Context) {
	_, ops, finishOperation := s.opsOf(c)
	if ops == nil {
		return
	}
	...
	if path == "" {
		respondError(c, http.StatusBadRequest, apiview.ErrMissingQuery, "missing file path (metadata.path or ?path=)")
		return
	}
...
func (s *Handler) filesDownload(c *gin.Context) {
	_, ops, finishOperation := s.opsOf(c)
	if ops == nil {
		return
	}
...
func (s *Handler) dirList(c *gin.Context) {
	_, ops, finishOperation := s.opsOf(c)
	if ops == nil {
		return
	}
...
func (s *Handler) dirCreate(c *gin.Context) {
	_, ops, finishOperation := s.opsOf(c)
	if ops == nil {
		return
	}
	...
	p := dirPathParam(c)
	if p == "" {
		respondError(c, http.StatusBadRequest, apiview.ErrMissingQuery, "missing 'path'")
		return
	}
...
func (s *Handler) dirDelete(c *gin.Context) {
	_, ops, finishOperation := s.opsOf(c)
	if ops == nil {
		return
	}
