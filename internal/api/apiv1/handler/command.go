...
func (s *Handler) resolveCwd(c *gin.Context, fs *bedfs.FS, cwd string) (string, bool) {
	if cwd == "" {
		return "", true
	}
...
func (s *Handler) runCommand(c *gin.Context) {
	...
	if req.Background {
		...
		sse := newSSE(c)
		sse.send(apiview.StreamEvent{Type: apiview.EventExecutionStart, ExecutionID: execution.ID})
		return
	}
	...
	execution, err := s.mgr.StartExecution(c.Request.Context(), b, bed.ExecutionForeground, req.Command, cwdInBed, req.Stdin, req.Envs, timeout, func(status bed.ExecutionStatus) {
		...
		eventType := apiview.EventStdout
		if output.Stream == bed.StreamStderr {
			eventType = apiview.EventStderr
		}
		sequence := output.Sequence
		sse.send(apiview.StreamEvent{Type: eventType, ExecutionID: startedExecutionID, Sequence: &sequence, Text: output.Text})
	})
	if err != nil {
		runtimeError(c, err.Error())
		return
...
func (s *Handler) interruptCommand(c *gin.Context) {
	id := c.Query("id")
	if id == "" {
		respondError(c, http.StatusBadRequest, apiview.ErrMissingQuery, "missing query parameter 'id'")
		return
	}
	execution, ok := s.mgr.Executions().Get(id)
	if !ok {
		respondError(c, http.StatusNotFound, apiview.ErrCommandNotFound, "command not found: "+id)
		return
	}
...
func (s *Handler) commandStatus(c *gin.Context) {
	execution, ok := s.mgr.Executions().Get(c.Param("id"))
	if !ok {
		respondError(c, http.StatusNotFound, apiview.ErrCommandNotFound, "command not found")
		return
	}
...
func (s *Handler) commandLogs(c *gin.Context) {
	id := c.Param("id")
	cursor := int64(-1)
	...
	execution, ok := s.mgr.Executions().Get(id)
	if !ok {
		respondError(c, http.StatusNotFound, apiview.ErrCommandNotFound, "execution not found")
		return
	}
	output, next, running, truncated := execution.Logs(cursor)
	payload := make([]apiview.ExecutionOutput, 0, len(output))
	for _, item := range output {
		payload = append(payload, apiview.ExecutionOutputFrom(item))
	}
...
func executionStatusPayload(status bed.ExecutionStatus) gin.H {
	...
	if status.Result != nil {
		payload["result"] = apiview.ExecutionResultFrom(*status.Result)
	}
	return payload
}
