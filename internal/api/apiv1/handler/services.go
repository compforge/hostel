...
func (s *Handler) serviceBed(c *gin.Context) *bed.Resident {
...
func (s *Handler) serviceList(c *gin.Context) {
...
func (s *Handler) selectedService(c *gin.Context) (*bed.Resident, *service.Status) {
	b := s.serviceBed(c)
	if b == nil {
		return nil, nil
	}
	...
	respondError(c, http.StatusNotFound, apiview.ErrServiceUnavailable, "service not found")
	return nil, nil
}
func (s *Handler) serviceGet(c *gin.Context) {
	_, status := s.selectedService(c)
	if status != nil {
		c.JSON(http.StatusOK, status)
	}
}
func (s *Handler) serviceRestart(c *gin.Context) {
	...
	if err := s.mgr.Services().Restart(b.Bed, c.Param("service")); err != nil {
		respondError(c, http.StatusConflict, apiview.ErrServiceUnavailable, err.Error())
		return
	}
	c.Status(http.StatusAccepted)
}
func (s *Handler) serviceAccess(c *gin.Context) {
	b, _ := s.selectedService(c)
	if b == nil {
		return
	}
...
func (s *Handler) serviceHoldRelease(c *gin.Context) {
	b := s.serviceBed(c)
	if b == nil {
		return
	}
...
func (s *Handler) serviceLogs(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusNotFound, apiview.ErrCommandNotFound, err.Error())
		return
	}
