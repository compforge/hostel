...
func TestPingAndHealthz(t *testing.T) {
	...
	if h["bed_pressure_threshold_percent"] != float64(80) {
		t.Fatalf("/healthz bed pressure threshold = %v, want 80", h["bed_pressure_threshold_percent"])
	}
...
func TestStatus(t *testing.T) {
	...
	if body["schema_version"] != float64(8) {
...
func TestAbsolutePathAndCommandCwdShareBedHome(t *testing.T) {
	...
	for _, ev := range parseSSE(t, rec.Body.String()) {
		if ev.Type == apiview.EventStdout {
			output += ev.Text
		}
	}
...
func TestCommandCwdPreservesHeredocTerminatorAtEOF(t *testing.T) {
	...
	for _, event := range parseSSE(t, rec.Body.String()) {
		switch event.Type {
		case apiview.EventStdout:
			stdout += event.Text
		case apiview.EventStderr:
			stderr += event.Text
...
func parseSSE(t *testing.T, body string) []apiview.StreamEvent {
	t.Helper()
	var evs []apiview.StreamEvent
	for _, frame := range strings.Split(body, "\n\n") {
		frame = strings.TrimSpace(frame)
		if frame == "" {
			continue
		}
		var ev apiview.StreamEvent
...
func TestCommandForegroundSSE(t *testing.T) {
	...
	for _, ev := range evs {
		if ev.Type == apiview.EventExecutionStart && ev.ExecutionID != "" {
			sawStarted = true
		}
		if ev.Type == apiview.EventPing && ev.Text == "pong" {
			sawPing = true
		}
		if ev.Type == apiview.EventStdout && strings.Contains(ev.Text, "hostel-ok") {
			sawStdout = true
		}
		if ev.Type == apiview.EventExecutionEnd {
			sawFinished = true
...
func TestCommandForegroundPreservesStdoutAndStderr(t *testing.T) {
	...
	var stdout, stderr string
	var exitCode *int
	var result *apiview.ExecutionResult
	for _, ev := range parseSSE(t, rec.Body.String()) {
		switch ev.Type {
		case apiview.EventStdout:
			stdout += ev.Text
...
func TestLostExecutorResultDoesNotExposeTransportError(t *testing.T) {
	payload := apiview.ExecutionResultFrom(bed.ExecutionResult{
...
func TestSessionStatePersistsAcrossRuns(t *testing.T) {
	...
	run := func(cmd string) []apiview.StreamEvent {
...
func TestMaxPinnedBedsReportsPressureWithoutBackpressure(t *testing.T) {
	...
	// The high watermark is reported for scheduling but never rejects work.
	rec = do(t, s, http.MethodGet, "/files/info?path=/workspace", nil, map[string]string{BedHeader: "two"})
	if rec.Code != http.StatusOK {
		t.Fatalf("work above pinned high watermark = %d %s", rec.Code, rec.Body.String())
	}
...
// every local Bed — managed in-memory Beds and orphaned/legacy luggage.
...
func TestBedListEndpoint(t *testing.T) {
	...
	if body.Instance.Sync != "noop" || body.Instance.OccupiedBeds != 2 || body.Instance.ResidentBeds != 2 ||
		...
		body.Instance.PhaseCounts["purging"] != 0 || body.Instance.PhaseCounts["dormant"] != 0 ||
	...
	if statuses["inv-live"] != "resident/active" || statuses["inv-idle"] != "resident/idle" {
		t.Fatalf("bed statuses = %v, want resident/active and resident/idle", statuses)
	}
	if _, ok := statuses["inv-cold"]; ok {
		t.Fatalf("normally evicted noop bed remained in inventory: %v", statuses)
	}

	// Evict every resident Bed. Noop follows the same lifecycle as a durable
	// Store; it simply has no snapshot to retain.
	finishOperation()
	...
	if err := json.Unmarshal(rec.Body.Bytes(), &released); err != nil {
		t.Fatalf("unmarshal released: %v", err)
	}
	if released.Instance.Status != "releasable" || len(released.Beds) != 0 {
		t.Fatalf("instance after evicting all = status:%s beds:%+v, want releasable and empty",
			released.Instance.Status, released.Beds)
	}
}
