...
func (s *Handler) networkPolicy(c *gin.Context) {
	...
	if c.Request.Method != http.MethodGet {
		...
		if err != nil {
			...
			respondError(c, status, apiview.ErrInvalidRequest, "invalid network policy body")
			return
		}
		...
		if err != nil {
			respondError(c, http.StatusBadRequest, apiview.ErrInvalidRequest, "invalid network policy body")
			return
		}
	}
	...
	if err != nil {
		...
		default:
			...
			respondError(c, http.StatusInternalServerError, apiview.ErrRuntimeError, "network policy update failed")
		}
		return
	}
	if c.Request.Method != http.MethodGet {
