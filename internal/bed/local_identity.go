...
func (m *Manager) localIdentityLocked(id string) *localIdentity {
	...
	local := &localIdentity{id: id, privilege: m.privileges.Bind(id, []string{
...
func (m *Manager) recoverLocalIdentities() error {
	...
	for _, local := range m.localIdentities {
		if err := local.privilege.Recover(context.Background()); err != nil {
...
func (m *Manager) removeIdentityTrees(ctx context.Context, local *localIdentity, removeRoot bool) error {
	...
	forget := lifecycle.NewSequence(lifecycle.Forget, lifecycle.Participant{Name: "privilege", Lifecycle: local.privilege})
	if err := forget.Run(ctx); err != nil {
		return err
	}
	m.mu.Lock()
	if m.localIdentities[local.id] == local {
