...
func newIdentityTestManager(t *testing.T, root string) *Manager {
	...
	manager, err := privilege.NewManager(privilege.BedUserReport{Strategy: "per_bed", UIDMin: 200000, UIDMax: 200000}, privilege.CurrentBedUser(), 0)
	if err != nil {
		t.Fatal(err)
	}
...
func claimTestUser(t *testing.T, m *Manager, id string) *localIdentity {
	...
	m.mu.Unlock()
	if err := local.privilege.Prepare(context.Background()); err != nil {
		t.Fatal(err)
	}
	return local
}
func assertUserReserved(t *testing.T, m *Manager) {
	t.Helper()
	binding := m.privileges.Bind("other", nil)
	if err := binding.Prepare(context.Background()); err == nil {
...
func TestPurgeRetainsUIDUntilFailedGCCleanupCompletes(t *testing.T) {
	...
	if err := m.privileges.Bind("other", nil).Prepare(context.Background()); err != nil {
		t.Fatal(err)
	}
}
...
func TestRecoveryFencesGCResidueWithWatermarksDisabled(t *testing.T) {
	...
	if err := m.privileges.Bind("other", nil).Prepare(ctx); err != nil {
		t.Fatal(err)
	}
}
...
func TestStaleIdentityCleanupCannotForgetReplacement(t *testing.T) {
	...
	if err := old.privilege.Forget(context.Background()); err != nil {
		t.Fatal(err)
	}
	assertUserReserved(t, m)
