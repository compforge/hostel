...
func (m *Manager) CreateShell(b *managedBed, cwdInBed string) (string, error) {
	...
	bedExecutor, err := m.executorManager.For(context.Background(), b.Bed)
	if err != nil {
		return "", err
	}
...
func (m *Manager) ForegroundShell(b *managedBed) (*Shell, error) {
	m.touchBed(b)
	b.mu.Lock()
...
func (b *managedBed) GetShell(id string) (*Shell, bool) {
	b.mu.Lock()
	defer b.mu.Unlock()
	sh, ok := b.shells[id]
	if !ok || sh.Dead() {
...
func (b *managedBed) DeleteShell(id string) bool {
	b.mu.Lock()
	sh, ok := b.shells[id]
	if ok {
		delete(b.shells, id)
