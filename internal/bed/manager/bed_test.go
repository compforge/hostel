...
func TestManagerFallsBackToShWhenBashMissing(t *testing.T) {
	root := t.TempDir()
	m, err := NewManager(hostfacts.Collect(), root, "default", filepath.Join(root, "missing-bash"), isolation.New(hostfacts.Collect(), "shared", root), nil, 0, nil)
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
...
func TestResolveDefaultBedAndValidation(t *testing.T) {
	...
	if b2.Name != "conv-123" || b2.Workdir() == b.Workdir() {
		t.Fatalf("distinct bed expected, got %+v", b2)
	}
	if got := m.ResidentBedCount(); got != 1 {
		t.Fatalf("ResidentBedCount = %d, want 1 tenant bed", got)
	}
}
...
func TestManagerCloseReleasesBedAmenityState(t *testing.T) {
	...
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), registry, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLifecycleObservations(t *testing.T) {
	root := t.TempDir()
	fs := newFakeStore()
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 0, store.NewManagerWithStores(fs))
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
...
func TestDeleteBedReleasesAndRemoves(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(b.Workdir(), "gone.txt"), []byte("gone"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(b.Spec().Dir); !os.IsNotExist(err) {
		t.Fatalf("noop eviction should remove the local Bed directory: %v", err)
	}
	resumed, err := m.Ensure(context.Background(), "conv-x")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(resumed.Workdir(), "gone.txt")); !os.IsNotExist(err) {
		t.Fatalf("noop resume should start fresh: %v", err)
	}
}
...
func TestMaxBedsCap(t *testing.T) {
	root := t.TempDir()
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 2, nil)
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
...
func TestMaxPinnedBedsHighWatermarkDoesNotReject(t *testing.T) {
	root := t.TempDir()
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 3, store.NewManagerWithStores(newFakeStore()))
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("admit new guest above pinned high watermark: %v", err)
	}
	finishNewGuest, err := m.BeginOperation(newGuest, OpExec, 0)
	if err != nil {
		t.Fatalf("start new guest above pinned high watermark: %v", err)
	}
	if got := m.PinnedBedCount(); got != 2 {
		t.Fatalf("pinned beds above high watermark = %d, want 2", got)
	}
	if !m.BedPressure() {
		t.Fatal("manager did not report bed pressure above pinned high watermark")
	}
	...
	finishNewGuest()
	if got := m.PinnedBedCount(); got != 2 {
		t.Fatalf("dirty idle guests should remain pinned, got %d", got)
	}
...
func TestMaxPinnedBedsResolution(t *testing.T) {
	root := t.TempDir()
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 3, nil)
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
	...
	unlimited, err := NewManager(hostfacts.Collect(), unlimitedRoot, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", unlimitedRoot), nil, 0, nil)
	if err != nil {
...
func TestMaxPinnedBedsConcurrentWorkCanExceedHighWatermark(t *testing.T) {
	...
	for _, candidate := range []*managedBed{a, b} {
		go func(candidate *managedBed) {
			<-start
	...
	for range 2 {
		result := <-results
		if result.err != nil {
			t.Fatalf("work above pinned high watermark: %v", result.err)
		}
		admitted++
		finishes = append(finishes, result.finish)
	}
	if admitted != 2 || m.PinnedBedCount() != 2 || !m.BedPressure() {
		t.Fatalf("concurrent work: admitted=%d pinned=%d pressure=%v", admitted, m.PinnedBedCount(), m.BedPressure())
	}
	for _, finish := range finishes {
		finish()
	}
	if got := m.PinnedBedCount(); got != 2 {
		t.Fatalf("dirty beds after finish = %d, want 2", got)
	}
...
func TestBedPressureStartsAtEightyPercentWithoutAdmissionLimit(t *testing.T) {
	...
	if m.BedPressure() {
		t.Fatal("7/10 pinned beds should remain below pressure")
	}
	finish, beginErr := m.BeginOperation(beds[7], OpExec, 0)
	if beginErr != nil {
		t.Fatalf("BeginOperation bed 7: %v", beginErr)
	}
	finishes = append(finishes, finish)
	if !m.BedPressure() {
	...
	for i := 8; i < 11; i++ {
		finish, beginErr := m.BeginOperation(beds[i], OpExec, 0)
		if beginErr != nil {
			t.Fatalf("high watermark rejected bed %d: %v", i, beginErr)
		}
		finishes = append(finishes, finish)
	}
	if got := m.PinnedBedCount(); got != 11 {
		t.Fatalf("pinned beds beyond high watermark = %d, want 11", got)
	}
}

func TestBedPressureIncludesOccupiedCapacityAndUsesConfiguredThreshold(t *testing.T) {
	root := t.TempDir()
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 4, nil)
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
	if err := m.SetBedPressureThresholdPercent(50); err != nil {
		t.Fatalf("SetBedPressureThresholdPercent: %v", err)
	}
	if _, err := m.Ensure(context.Background(), "one"); err != nil {
		t.Fatal(err)
	}
	if m.BedPressure() {
		t.Fatal("1/4 occupied beds should remain below the configured 50% watermark")
	}
	if _, err := m.Ensure(context.Background(), "two"); err != nil {
		t.Fatal(err)
	}
	if !m.BedPressure() {
		t.Fatal("2/4 occupied beds should report pressure at the configured 50% watermark")
	}
	if got := m.PinnedBedCount(); got != 0 {
		t.Fatalf("clean occupied beds unexpectedly pinned: %d", got)
	}
	if got := m.OccupiedBedCount(); got != 2 {
		t.Fatalf("OccupiedBedCount = %d, want 2", got)
	}
	if got := m.BedPressureThresholdPercent(); got != 50 {
		t.Fatalf("BedPressureThresholdPercent = %d, want 50", got)
	}
	for _, invalid := range []int{-1, 101} {
		if err := m.SetBedPressureThresholdPercent(invalid); err == nil {
			t.Fatalf("SetBedPressureThresholdPercent(%d): want error", invalid)
		}
	}
}
...
func TestNoopDataDoesNotRemainPinned(t *testing.T) {
	root := t.TempDir()
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 1, nil)
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
	if err := m.SetMaxPinnedBeds(1); err != nil {
...
func TestStoreSyncTriggerReleasesPinnedBed(t *testing.T) {
	root := t.TempDir()
	fs := newFakeStore()
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 1, store.NewManagerWithStores(fs))
	if err != nil {
		t.Fatalf("NewManager: %v", err)
	}
...
type fakeStore struct {
	...
	metas              map[string][]byte // bedID → persisted meta.json
...
func TestInitializeBedRunsStoreWorkAsynchronouslyAndReservesCapacity(t *testing.T) {
	...
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 1, store.NewManagerWithStores(backend))
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := m.OccupiedBedCount(); got != 1 {
		t.Fatalf("OccupiedBedCount during initialization = %d, want 1", got)
	}
	if got := m.ResidentBedCount(); got != 0 {
		t.Fatalf("ResidentBedCount during initialization = %d, want 0", got)
	}
	if !m.BedPressure() {
		t.Fatal("initializing bed that occupies the only max_beds slot should report pressure")
	}
...
func TestInitializeBedRetainsFailureReason(t *testing.T) {
	...
	if m.OccupiedBedCount() != 0 {
		t.Fatalf("failed initialization retained occupied count %d", m.OccupiedBedCount())
	}
}
...
func TestDurableEvictRemovesLocalCopyAndColdResumes(t *testing.T) {
	...
	// Write data into a bed, evict it, then require the next placement to
	// restore from the durable snapshot rather than retain node-local state.
	b, _ := m.Ensure(context.Background(), "conv-1")
	if err := os.WriteFile(filepath.Join(b.Workdir(), "data.txt"), []byte("payload"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(b.Spec().Dir); !os.IsNotExist(err) {
		t.Fatalf("durable local copy should be removed after evict: %v", err)
	}
	if string(fs.snaps["conv-1"]) != "payload" {
		t.Fatalf("snapshot content = %q", fs.snaps["conv-1"])
	}
	// Re-resolve the same bed id through the snapshot path.
	b2, err := m.Ensure(context.Background(), "conv-1")
	if err != nil {
		t.Fatalf("re-Resolve: %v", err)
	}
	if got := m.PinnedBedCount(); got != 0 {
		t.Fatalf("restored clean bed is pinned: %d", got)
	}
	if record := b2.Lifecycle().LastInitialization; record == nil || record.Source != "snapshot" {
		t.Fatalf("cold initialization = %+v, want snapshot", record)
	}
	if restored, err := os.ReadFile(filepath.Join(b2.Workdir(), "restored.txt")); err != nil || string(restored) != "payload" {
		t.Fatalf("cold resume payload = %q err=%v", restored, err)
	}
}
...
func TestStaleLuggageDiscardedOnResume(t *testing.T) {
	root := t.TempDir()
	fs := newFakeStore()
	m, _ := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 0, store.NewManagerWithStores(fs))

	b, _ := m.Ensure(context.Background(), "conv-s")
	_ = os.WriteFile(filepath.Join(b.Workdir(), "data.txt"), []byte("old"), 0o644)
	if ok, err := m.Evict(context.Background(), "conv-s"); err != nil || !ok {
		t.Fatalf("Evict: ok=%v err=%v", ok, err)
	}
	// Simulate luggage left by an older Hostel version.
	if err := os.MkdirAll(filepath.Join(b.Spec().Dir, "data", "workspace"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := saveMeta(b.Spec().Dir, bedMeta{Version: 1, BedID: "conv-s", Generation: 1}); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(b.Spec().Dir, "data", "workspace", "data.txt"), []byte("old"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(b2.Workdir(), "restored.txt")); err != nil {
		t.Fatalf("stale luggage should be replaced by a restore: %v", err)
	}
	if _, err := os.Stat(filepath.Join(b2.Workdir(), "data.txt")); err == nil {
...
func TestColdResumeRestoresFromSnapshot(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(b.Workdir(), "data.txt"), []byte("payload"), 0o644)
...
func TestPersistDirty(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(b.Workdir(), "data.txt"), []byte("v1"), 0o644)
...
func TestPersistKeepsActivityAfterSnapshotPinned(t *testing.T) {
	...
	m, _ := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 1, store.NewManagerWithStores(fs))
...
func TestEvictCanceledByActivity(t *testing.T) {
	...
	m, _ := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 0, store.NewManagerWithStores(ss))
...
func TestPurgeEndsIdentity(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(b.Workdir(), "data.txt"), []byte("x"), 0o644)
	if ok, _ := m.Evict(context.Background(), "conv-p"); !ok {
		t.Fatal("evict failed")
	}
	if info, _ := fs.Stat(context.Background(), "conv-p"); info == nil {
		t.Fatal("snapshot should exist after evict")
	}
	// Purge the evicted identity: snapshot gone, resolve starts fresh.
	...
	if _, err := os.Stat(b.Spec().Dir); !os.IsNotExist(err) {
		t.Fatal("local directory should remain absent after purge")
	}
...
func TestPurgeJoinsCanceledInitializationBeforeDeleting(t *testing.T) {
	...
	m, err := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 0, store.NewManagerWithStores(backend))
	if err != nil {
		t.Fatal(err)
	}
...
func writeLegacyLuggage(t *testing.T, root, id string, size int, generation int64, lastActiveAt time.Time) {
	t.Helper()
	...
	if err := os.WriteFile(filepath.Join(workspace, "data.txt"), make([]byte, size), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := saveMeta(dir, bedMeta{
		Version:      1,
		BedID:        id,
...
// Luggage GC remains for directories orphaned by an unclean shutdown or left
// by older Hostel versions. Over the high watermark it deletes stale
// generations first, then LRU entries, until under the low watermark.
func TestCollectLuggageWatermarks(t *testing.T) {
	...
	writeLegacyLuggage(t, root, "conv-mid", 10_000, 1, now.Add(-2*time.Hour))
	writeLegacyLuggage(t, root, "conv-new", 10_000, 1, now.Add(-time.Hour))
...
func TestCollectLuggageStaleFirst(t *testing.T) {
	...
	writeLegacyLuggage(t, root, "conv-b", 10_000, 1, now.Add(-time.Hour))
	fs.mu.Lock()
	fs.snaps["conv-a"] = []byte("a")
	fs.snaps["conv-b"] = []byte("b")
	fs.gens["conv-a"] = 1
	fs.gens["conv-b"] = 1
	fs.mu.Unlock()
...
// Inventory reports resident beds, but a normally evicted durable Bed no
// longer has node-local state to report.
func TestInventory(t *testing.T) {
	...
	if cold, ok := byID["conv-cold"]; ok {
		t.Fatalf("durably evicted bed remained in local inventory: %+v", cold)
	}
}
...
// Usage counters accumulate in memory, travel with the durable snapshot, and
// keep accumulating after a cold resume.
func TestProfileAccumulatesAndSurvivesEvict(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "conv-prof")); !os.IsNotExist(err) {
		t.Fatalf("durable local copy should be removed: %v", err)
	}

	// Cold resume seeds from the snapshot meta and keeps counting on top.
...
// Restore duration is measured on the node that performs the cold resume.
func TestProfileRecordsMigrationCost(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "conv-cost")); !os.IsNotExist(err) {
		t.Fatalf("durable local copy should be removed: %v", err)
	}

	// The restore is timed on this host.
...
func TestBedDirLayoutAndMetaAcrossRestart(t *testing.T) {
	root := t.TempDir()
	m, _ := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 0, nil)

	b, _ := m.Ensure(context.Background(), "default")
	// Layout: {root}/default/{meta.json,data/workspace}; Home is the bed_home
	// root (data), Workspace the real subdir below it.
	if b.Rootfs() != filepath.Join(root, "default", "data") {
		t.Fatalf("Home = %s", b.Rootfs())
	}
	if b.Workdir() != filepath.Join(root, "default", "data", "workspace") {
		t.Fatalf("Workspace = %s", b.Workdir())
	}
	if _, err := os.Stat(filepath.Join(b.Spec().Dir, "meta.json")); err != nil {
		t.Fatalf("meta.json missing: %v", err)
	}
	created := b.Spec().CreatedAt

	// "Restart": a new Manager over the same root sees the same identity.
	time.Sleep(5 * time.Millisecond)
	m2, _ := NewManager(hostfacts.Collect(), root, "default", "/bin/bash", isolation.New(hostfacts.Collect(), "shared", root), nil, 0, nil)
