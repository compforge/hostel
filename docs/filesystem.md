...
| Executor | shared/confined 下由 helper 或 Carrier 提供视图；private 下为内部 bed_home 挂载或 `/workspace` | BedFS `View` 定义语义，isolation 实现投影 |
...
`capabilities.workspace_mount` 只说明进程里是否存在 private 文件视图的真实 `/workspace` mount，不表示整个 Bed 为 suite 或 BedFS 是否可用。`workspace_view.mode` 报告实际进程视图：`mount`、`proot`、`pathshim` 或 `carrier`；`available=false` 与 `reason` 表示没有用户态 helper 通过选择验证。BedFS 的结构化路径映射是所有房型的基础能力。
