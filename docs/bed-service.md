...
Service 维护启动中、就绪、等待重启、失败和停止等运行状态，同时保留进程退出事实。
就绪由声明的检查判断；只有进程检查时，仅证明进程仍运行，不保证业务已初始化。
必需服务失去就绪时更新 Bed readiness，但诊断、重启与回收入口仍可访问。
...
2. 将地址注入服务，确认目标进程实际绑定成功，再执行就绪检查。
3. 若有明确绑定冲突，确认本次启动的进程树退出，释放对应 allocation，换端口有限重试。
   其他启动错误按实际原因报告，不能全部归类为端口冲突。
4. 只有绑定及就绪均成功才发布 endpoint；端口池耗尽或重试预算耗尽时明确失败。
...
- 外部进程抢占、端口池耗尽、绑定失败重试不误报 Ready，不把其他 HTTP 服务当作本次启动结果。
